...
<h2 style="color: darkblue;font size=12"> Climate Monitoring Brief: St. Thomas and St. John</h2>
--------
...
N. Besemer, A. Palacio, T.Gill, K. Simmons,  M. Studivan, I. Enochs - January 2024
</h5>
...
The AOML Coral Program tracks the status and trends of coral reef ecosystems in the U.S. Atlantic and Caribbean as part of the National Coral Reef Monitoring Program (NCRMP). This summary brief provides an overview of the most recent climate monitoring efforts in Saint Thomas and Saint John (US Virgin Islands).


### Expedition summary

* The NCRMP Atlantic climate monitoring team recently traveled to St. Thomas and St. John from July 19th to July 29th, 2023
* XX different sites were visited by four team members and collaborators completing a total of XX dives. 
...
Subsurface temperature recorders (STRs) were recovered and redeployed at all nine transects, each one composed by four depths (Fig. 1). In total, more than 7.7 million temperature observations were collected from 24 instruments (Table 1)
...
<font size="2"> **Figure 2:** Temperature conditions at seven transects representing a depth gradient (1m, 5m, 15m and 25m). Data were collected from August 2017 to March 2022. The COVID 19 pandemic caused a significant delay in collection of these instruments and some of them had stopped recording. Contractors were hired to complete the work in 2021 as travel was not allowed. Some instruments survived the entire deployment but others have not. </font>
...
pH_Temp_Plot<-ggplot(data = pH) +
  ...
  annotate("rect",
          xmin = seq.POSIXt(as.POSIXct('2023-07-24 18:56:00', tz='America/Puerto_Rico'),
                         by = "day", length.out = 3),
          xmax = seq.POSIXt(as.POSIXct('2023-07-25 05:53:00', tz='America/Puerto_Rico'),
                         by = "day", length.out = 3),
          ymin = -Inf, ymax = Inf,
          fill="grey", alpha=0.5) +
   ...
   annotate("text",
          ...
          x = seq.POSIXt(as.POSIXct('2023-07-24 11:00:00', tz='America/Puerto_Rico'),
                         by = "day", length.out = 4),
          y = pH_max + 0.02)+ 
  # Data
  ...
  # Axes
  scale_y_continuous(limits = c(7.36, 7.99),
                     name="pH (Total scale)",
  ...
  coord_cartesian(ylim = c(7.36, 7.99),
...
<font size="2">**Figure 3:** Data	from XX diurnal suite monitoring from July 24th to July 27th. Top panel: pH and	temperature fromSeaFET. Bottom panel: Photosynthetically	Available	Radiation (PAR)	and current speed from EcoPAR and Tiltmeter. Shading denotes nighttime throughout	sequence of the plot. Instruments measured parameters every 15 minutes. </font>
...
```{r CAUs, echo=FALSE, out.width = '50%', fig.cap="Figure 5: CAU and BMU pair after deployment. CAUs are two parallel PVC plates to quantify settled accretors. BMU is mounted coral skeleton installed at the base of the metal stake and has been encrusted.", fig.align='center'}

knitr::include_graphics("Data/Pictures/newCAUBMU.JPG")
